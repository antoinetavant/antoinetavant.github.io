(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      var element = document.getElementById("7132289f-de3c-4527-bd7b-cf2ccc31cda3");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '7132289f-de3c-4527-bd7b-cf2ccc31cda3' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        const hashes = {"https://cdn.bokeh.org/bokeh/release/bokeh-2.3.0.min.js": "HjagQp6T0/7bxYTAXbLotF1MLAGWmhkY5siA1Gc/pcEgvgRPtMsRn0gQtMwGKiw1", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.3.0.min.js": "ZEPPTjL+mdyqgIq+/pl9KTwzji8Kow2NnI3zWY8+sFinWP/SYJ80BnfeJsa45iYj", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.3.0.min.js": "exLqv2ACDRIaV7ZK1iL8aGzGYQvKVuT3U2CT7FsQREBxRah6JrkVCoFy0koY1YqV"};
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          if (url in hashes) {
            element.crossOrigin = "anonymous";
            element.integrity = "sha384-" + hashes[url];
          }
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.3.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.3.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.3.0.min.js"];
      var css_urls = [];
      
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"17150eb1-d767-4099-a434-07c2187e5c43":{"defs":[{"extends":null,"module":null,"name":"DataModel","overrides":[],"properties":[]}],"roots":{"references":[{"attributes":{"children":[{"id":"1433"},{"id":"1325"}],"max_height":600,"max_width":750,"min_height":400,"sizing_mode":"stretch_both"},"id":"1434","type":"Column"},{"attributes":{"end":100,"height":40,"js_property_callbacks":{"change:value":[{"id":"1429"}]},"sizing_mode":"fixed","start":0,"title":"Total Technological Optimisation factor in 2050 (%) ","value":50,"width":200},"id":"1426","type":"Slider"},{"attributes":{"args":{"opt_op":{"id":"1427"},"opt_tech":{"id":"1426"},"source":{"id":"1324"}},"code":"\\n    const data = source.data;\\n    const tech_percent = opt_tech.value;\\n    const op_perc = opt_op.value;\\n\\n    const x = data[&#x27;x&#x27;]\\n    const tech_traj = data[&#x27;tech_traj&#x27;]\\n    const ref_traj = data[&#x27;ref_traj&#x27;]\\n    const oper_trj = data[&#x27;oper_trj&#x27;]\\n    \\n    const factor_op = 1-(1-op_perc/100) ** (1/ (x.length) )\\n    const factor_tech = 1-(1-tech_percent/100) ** (1/ (x.length) )\\n    \\n    for (var i = 0; i &lt; x.length; i++) {\\n        oper_trj[i] = ref_traj[i] * (1 - factor_op) ** (i) ;\\n    }\\n    \\n    for (var i = 0; i &lt; x.length; i++) {\\n        tech_traj[i] = oper_trj[i] * (1 - factor_tech) ** (i) ;\\n    }\\n    \\n    source.change.emit();\\n"},"id":"1429","type":"CustomJS"},{"attributes":{"axis_label":"Emission [MtCO2]","formatter":{"id":"1364"},"major_label_policy":{"id":"1366"},"ticker":{"id":"1339"}},"id":"1338","type":"LinearAxis"},{"attributes":{"axis":{"id":"1338"},"dimension":1,"ticker":null},"id":"1341","type":"Grid"},{"attributes":{"children":[{"id":"1426"},{"id":"1427"}],"height":100,"sizing_mode":"fixed","width":200},"id":"1431","type":"Column"},{"attributes":{"label":{"value":"optimisation Operation"},"renderers":[{"id":"1395"}]},"id":"1408","type":"LegendItem"},{"attributes":{},"id":"1339","type":"BasicTicker"},{"attributes":{},"id":"1326","type":"DataRange1d"},{"attributes":{},"id":"1344","type":"WheelZoomTool"},{"attributes":{},"id":"1343","type":"PanTool"},{"attributes":{"children":[{"id":"1431"},{"id":"1432"}],"height":100,"sizing_mode":"fixed","width":400},"id":"1433","type":"Row"},{"attributes":{"active_drag":null,"active_multi":null,"tools":[{"id":"1342"},{"id":"1343"},{"id":"1344"},{"id":"1345"},{"id":"1346"},{"id":"1347"}]},"id":"1349","type":"Toolbar"},{"attributes":{},"id":"1342","type":"CrosshairTool"},{"attributes":{"overlay":{"id":"1348"}},"id":"1345","type":"BoxZoomTool"},{"attributes":{},"id":"1364","type":"BasicTickFormatter"},{"attributes":{},"id":"1346","type":"SaveTool"},{"attributes":{"source":{"id":"1324"}},"id":"1361","type":"CDSView"},{"attributes":{},"id":"1366","type":"AllLabels"},{"attributes":{},"id":"1347","type":"ResetTool"},{"attributes":{"line_alpha":0.1,"line_color":"red","line_width":3,"x":{"field":"x"},"y":{"field":"ref_traj"}},"id":"1359","type":"Line"},{"attributes":{"line_alpha":0.6,"line_color":"green","line_width":3,"x":{"field":"x"},"y":{"field":"tech_traj"}},"id":"1410","type":"Line"},{"attributes":{"line_alpha":0.9,"line_color":"red","line_width":3,"x":{"field":"x"},"y":{"field":"ref_traj"}},"id":"1358","type":"Line"},{"attributes":{"line_alpha":0.6,"line_color":"#1f77b4","line_width":3,"x":{"field":"x"},"y":{"field":"oper_trj"}},"id":"1393","type":"Line"},{"attributes":{"line_alpha":0.1,"line_color":"green","line_width":3,"x":{"field":"x"},"y":{"field":"tech_traj"}},"id":"1411","type":"Line"},{"attributes":{"label":{"value":"Reference +4%/yr"},"renderers":[{"id":"1360"}]},"id":"1374","type":"LegendItem"},{"attributes":{"line_alpha":0.6,"line_dash":[6],"line_width":3,"x":{"field":"x"},"y":{"field":"objectif_traj"}},"id":"1376","type":"Line"},{"attributes":{"label":{"value":"optimisation Technologique"},"renderers":[{"id":"1412"}]},"id":"1425","type":"LegendItem"},{"attributes":{},"id":"1335","type":"BasicTicker"},{"attributes":{"source":{"id":"1324"}},"id":"1413","type":"CDSView"},{"attributes":{"click_policy":"mute","items":[{"id":"1374"},{"id":"1391"},{"id":"1408"},{"id":"1425"}],"label_text_font_size":"8pt","location":"top_left"},"id":"1373","type":"Legend"},{"attributes":{},"id":"1367","type":"BasicTickFormatter"},{"attributes":{"text":"Evolution of the World total emission"},"id":"1362","type":"Title"},{"attributes":{},"id":"1369","type":"AllLabels"},{"attributes":{"data_source":{"id":"1324"},"glyph":{"id":"1393"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"1394"},"view":{"id":"1396"}},"id":"1395","type":"GlyphRenderer"},{"attributes":{},"id":"1370","type":"UnionRenderers"},{"attributes":{},"id":"1371","type":"Selection"},{"attributes":{"source":{"id":"1324"}},"id":"1396","type":"CDSView"},{"attributes":{"bottom_units":"screen","fill_alpha":0.5,"fill_color":"lightgrey","left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"1348","type":"BoxAnnotation"},{"attributes":{"label":{"value":"Objectif -3.39%/yr"},"renderers":[{"id":"1378"}]},"id":"1391","type":"LegendItem"},{"attributes":{"data_source":{"id":"1324"},"glyph":{"id":"1376"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"1377"},"view":{"id":"1379"}},"id":"1378","type":"GlyphRenderer"},{"attributes":{"background_fill_color":"#fafafa","below":[{"id":"1334"}],"center":[{"id":"1337"},{"id":"1341"},{"id":"1373"}],"height":500,"left":[{"id":"1338"}],"renderers":[{"id":"1360"},{"id":"1378"},{"id":"1395"},{"id":"1412"}],"sizing_mode":"stretch_both","title":{"id":"1362"},"toolbar":{"id":"1349"},"toolbar_location":"below","toolbar_sticky":false,"width":850,"x_range":{"id":"1326"},"x_scale":{"id":"1330"},"y_range":{"id":"1328"},"y_scale":{"id":"1332"}},"id":"1325","subtype":"Figure","type":"Plot"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","line_width":3,"x":{"field":"x"},"y":{"field":"oper_trj"}},"id":"1394","type":"Line"},{"attributes":{"line_alpha":0.1,"line_dash":[6],"line_width":3,"x":{"field":"x"},"y":{"field":"objectif_traj"}},"id":"1377","type":"Line"},{"attributes":{"source":{"id":"1324"}},"id":"1379","type":"CDSView"},{"attributes":{},"id":"1328","type":"DataRange1d"},{"attributes":{"end":30,"height":40,"js_property_callbacks":{"change:value":[{"id":"1429"}]},"sizing_mode":"fixed","start":0,"title":"Total Operation Optimisation factor in 2050 (%) ","value":10,"width":200},"id":"1427","type":"Slider"},{"attributes":{"data_source":{"id":"1324"},"glyph":{"id":"1410"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"1411"},"view":{"id":"1413"}},"id":"1412","type":"GlyphRenderer"},{"attributes":{"axis":{"id":"1334"},"ticker":null},"id":"1337","type":"Grid"},{"attributes":{"data_source":{"id":"1324"},"glyph":{"id":"1358"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"1359"},"view":{"id":"1361"}},"id":"1360","type":"GlyphRenderer"},{"attributes":{"data":{"objectif_traj":{"__ndarray__":"","dtype":"float64","order":"little","shape":[300]},"oper_trj":{"__ndarray__":"","dtype":"float64","order":"little","shape":[300]},"ref_traj":{"__ndarray__":"","dtype":"float64","order":"little","shape":[300]},"tech_traj":{"__ndarray__":"","dtype":"float64","order":"little","shape":[300]},"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[300]}},"selected":{"id":"1371"},"selection_policy":{"id":"1370"}},"id":"1324","type":"ColumnDataSource"},{"attributes":{},"id":"1330","type":"LinearScale"},{"attributes":{"axis_label":"year","formatter":{"id":"1367"},"major_label_policy":{"id":"1369"},"ticker":{"id":"1335"}},"id":"1334","type":"LinearAxis"},{"attributes":{"height":100,"sizing_mode":"fixed","width":200},"id":"1432","type":"Spacer"},{"attributes":{},"id":"1332","type":"LinearScale"}],"root_ids":["1434"]},"title":"Bokeh Application","version":"2.3.0"}}';
                  var render_items = [{"docid":"17150eb1-d767-4099-a434-07c2187e5c43","root_ids":["1434"],"roots":{"1434":"7132289f-de3c-4527-bd7b-cf2ccc31cda3"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();